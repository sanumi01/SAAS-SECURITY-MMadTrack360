...
export default function Sidebar() {
  return (
    <aside className="sidebar fixed right-0 top-0 h-screen w-[280px] bg-gradient-to-b from-slate-800 to-slate-900 border-l border-slate-700 z-50 shadow-xl">
      <div className="sidebar-header px-6 py-6 border-b border-slate-700 flex flex-col items-end">
        <div className="sidebar-logo flex items-center gap-2 text-cyan-400 font-bold text-xl">
          <ChartBarIcon className="w-6 h-6" />
          MMadTrack360
        </div>
      </div>
      <nav className="sidebar-menu py-4 flex flex-col gap-1 items-end">
        {menu.map(item => (
          <NavLink
            key={item.name}
            to={item.to}
            className={({ isActive }) =>
              `menu-item flex items-center gap-3 px-5 py-3 text-slate-400 text-sm transition-all duration-200 cursor-pointer ${isActive ? 'active bg-slate-800 text-cyan-400 border-r-4 border-cyan-400 font-semibold' : 'hover:bg-slate-800 hover:text-cyan-400 hover:border-r-4 hover:border-cyan-400'}`
            }
          >
